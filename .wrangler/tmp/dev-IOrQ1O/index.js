...
// .wrangler/tmp/bundle-B2cvRq/checked-fetch.js
...
// .wrangler/tmp/bundle-B2cvRq/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-B2cvRq/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-B2cvRq/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
async function openaiResponse(args, env) {
  try {
    ...
    try {
      ...
      return { error: `Failed to parse OpenAI response: ${e.message}` };
    }
    const outputText = typeof data.output_text === "string" && data.output_text.length ? data.output_text : (() => {
    ...
    return {
      error: error instanceof Error ? `Request failed: ${error.message}` : `Request failed: ${String(error)}`
    };
  }
}
...
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
async function testGraphQL(args, env) {
  try {
...
__name(testGraphQL, "testGraphQL");
...
var resolvers = {
  Query: {
    testGraphQL: async (_, args, context) => {
